...
pub fn sync(global: GlobalOptions, options: SyncOptions) -> Result<(), SyncError> {
    ...
    match api_client {
        Clients::OpenCloud(mut open_cloud) => session.populate_asset_cache(&mut open_cloud)?,
        Clients::RobloxApi(mut roblox_api) => session.populate_asset_cache(&mut roblox_api)?,
    }
